...
public class UrbanLadder_ProductDetails extends UrbanLadder_ListOfProductPage {
	...
	public static void verifypage() throws InterruptedException, IOException
	{
		driver=ReusableComponent.WebDriver();
		Thread.sleep(1000);
		driver.get(url);
		driver.manage().window().maximize();
		boolean val=driver.findElement(UrbanLadder_ProductDetailsUI.addtocart).isDisplayed();
		if(val==true)
		{
			Log.log2.info("Landed on product details page");
		}
		else
		{
			Date d = new Date();
